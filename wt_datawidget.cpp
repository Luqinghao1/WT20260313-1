 ...
 * 7. [最新优化] 数据保存增加原生 Excel (.xlsx, .xls) 支持，利用 QXlsx 将全表完美导出。
 */
...
void WT_DataWidget::saveAndLoadNewData(const QString& oldFilePath, const QStringList& headers, const QVector<QStringList>& fullTable)
{
    ...
    // 生成默认文件名
    QString defaultName = QString("%1_滤波采样_%2.xlsx").arg(baseName).arg(QDateTime::currentDateTime().toString("HHmmss"));
    ...
    if (isExcel) {
        ...
        xlsx.write(1, 1, "// PWT System: Data Processed by Multi-Stage Filter Module", infoFormat);
        ...
        out << "// PWT System: Data Processed by Multi-Stage Filter Module\n";
    ...
    ui->statusLabel->setText("滤波取样完成，保留有效点数: " + QString::number(fullTable.size()));
}
